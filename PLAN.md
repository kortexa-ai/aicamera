...
- Xcode automatic signing created the app-specific Developer ID provisioning profile and exported build 13 with the existing Developer ID Application identity. Apple notarization submission `e0e13b26-0391-44a1-84c1-33db5a34a2b8` was accepted; the ticket was stapled and validated, strict nested signature verification passed, and Gatekeeper reports `Notarized Developer ID`. The resulting ZIP is a distribution candidate, not yet the planned installer package.
