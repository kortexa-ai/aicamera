...
- Current development target: build 17. It reorganizes Settings into General, AI, and Privacy; adds configuration-aware privacy disclosures; and adds optional in-process HY-MT2 translation with separately downloaded, integrity-checked weights. Its bundled audio driver remains build 12 because this host-only change does not alter the validated HAL binary.
- The installed host is signed build 17 in `/Applications`. Its validated HAL driver remains build 12 because this host-only change does not alter or reinstall the driver.
