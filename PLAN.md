...
- Current development target: build 14. It refreshes LiveKit WebRTC to `144.7559.14`, updates the vendored three.js overlay runtime from r149 to r185, and adds the App Store video category metadata. Its bundled audio driver remains build 12 because this dependency-only change does not alter the validated HAL binary.
- The installed and active host remains the previously accepted build 13. Its protected install marker and app report build 13, and its validated HAL driver remains build 12. Build 14 has not been installed or activated.
