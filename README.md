...
AI Camera is a configurable native macOS audio/video proxy. It captures one hardware camera and microphone, applies local or remote AI stages, draws live annotations, mixes generated speech with the microphone, and publishes the result as **AI Camera** and **AI Camera Audio** devices.
...
- Bounded gesture, object-detection, VLM, ASR, agent, and TTS stages.
...
- Microphone forwarding, TTS injection, and barge-in.
...
5. Open Settings, select physical inputs, and choose **AI Camera Audio** as the mixed output.
6. Configure a JSON profile or select the optional **Kortexa local** preset.
7. Select **AI Camera** and **AI Camera Audio** in the client application, then start the proxy.

The profile is stored at `~/Library/Application Support/AI Camera/profile.json`. The full editor is in Settings. Checked examples are in [`Examples/`](Examples/).
